...
class VCWaterfallExpert(DeterministicSubtaskExpert):
    ...
    def _compute_safe(self, inp: SAFEConversionInput) -> SAFEConversionOutput:
        ...
        safe_pct = safe_inv / safe_cap
        sa_pct = sa_inv / (pre_money + sa_inv)
        founder_pct = D1 - safe_pct - sa_pct

        if founder_pct <= D0:
            raise ValueError("Founder ownership <= 0%. Check input parameters.")

        total_shares = D(str(founder)) / founder_pct
        safe_shares = total_shares * safe_pct
        sa_shares = total_shares * sa_pct
        price = sa_inv / sa_shares
        pre_money_verify = (D(str(founder)) + safe_shares) * price
        ...
        return SAFEConversionOutput(
            ...
            series_a_shares=float(sa_shares.quantize(D("0.01"), rounding=ROUND_HALF_UP)),
            price_per_share=float(price.quantize(D("0.0001"), rounding=ROUND_HALF_UP)),
